...
function HttpServer(options) {
  ...
  before.push(async function(req,res){ 
    const {appPath, appIndex, defaultCustomFolder, domains=[]} = global.config;
    ...
    let resourceUrl = appPath + req.url;//资源路径
    if(url==='/') resourceUrl = path.join(resourceUrl, appIndex);

    if(matchDomain){//如果当前域名（仲裁委）有定制
      let customUrl;
      if (!matchDomain.folder){
        customUrl = defaultCustomFolder + "/" + matchDomain.name + resourceUrl;;  // 根据租户名称默认的定制文件夹
      }else{
        customUrl = matchDomain.folder + resourceUrl;
      }

      await new Promise((resolve) => { // 查找文件
          fs.access(path.join(root, customUrl), fs.constants.F_OK, (err) => {
