...
function HttpServer(options) {
  ...
  before.push(async function(req,res){ 
    const {domains=[],appName2dir={},appName2indexDir={},customization={},defaultDir={}} = global.config;
    ...
    if(appMatch && appName2dir[appMatch[1]]!==undefined){
      ...
      appName = 'root';
      resourceUrl = url==='/'?appName2indexDir[appName]:url;
    }
    ...
    if(matchDomain){//如果当前域名（仲裁委）有定制
      ...
      if(!customization[matchDomain.name].appdir){//如果appdir不存在
