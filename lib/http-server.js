...
function HttpServer(options) {
  ...
  before.push(async function(req,res){ 
    const {domains=[],appDir,customization={},defaultDir={}} = global.config;
    ...
    if(appMatch && appDir[appMatch[1]]!==undefined){
      appName = appDir[appMatch[1]];
    ...
    if(defaultDir.appDir[appName]){
      ...
      const appFolder = appDir[appName];//子应用文件夹路径
      defaultPath = defaultDir.main + appFolder;
    }
    defaultPath += resourceUrl;

    //tod 定制路径
    let customPath = '';
    if(!customization[matchDomain.name]){
      customization[matchDomain.name] = {dir:`${defaultDir.custom}/${matchDomain.name}`};//如果没有设置[customization.xxx]，那么默认xxx的dir设置为：<defaultDir.custom>/xxx
      customPath = `${defaultDir.custom}/${matchDomain.name}`;
    }else if(){}
    !customization[matchDomain.name].dir&&(customization[matchDomain.name].dir = `${defaultDir.custom}/${matchDomain.name}`);
    if(customization[matchDomain.name]&&customization[matchDomain.name].appDir[appName]){
      customPath = customization[matchDomain.name].appDir[appName];
    }else{
      customPath = customization[matchDomain.name]
    }

    // const defaultPath =  defaultDir.main + appFolder + req.url;// 默认路径
    if(matchDomain){//如果当前域名（仲裁委）有定制
      customPath 
      const acFolder = '/' + matchDomain.name;//仲裁委文件夹路径
      const customPath = defaultDir.custom + acFolder + appFolder + req.url;// 定制的路径
