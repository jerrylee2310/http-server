...
function HttpServer(options) {
  ...
  before.push(async function(req,res){ 
    ...
    const appName=req.url&&req.url.match(/^\/(.*?)\//)?req.url.match(/^\/(.*?)\//)[1]:'';
    const appFolder = appDir[appName]||appDir['mis'];//子应用文件夹路径
    const matchDomain = domains.find(({domain})=>host === domain);
    ...
    if(matchDomain){//如果当前域名（仲裁委）有定制
      ...
      await new Promise((resolve) => {
        ...
        console.log(path.join(root, customPath));
